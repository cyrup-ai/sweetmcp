...
impl McpTool for IpTool {
    ...
    fn schema(builder: SchemaBuilder) -> Value {
        builder
            .required_enum(
                ...
                &["get_public_ip", "validate_ip", "ip_info", "is_private", "ip_to_binary", "create_ipv4", "create_ipv6", "cidr_contains"],
            )
            .optional_string("ip", "IP address to analyze (required for most operations)")
            .optional_string("cidr", "CIDR notation for subnet operations (e.g., '192.168.1.0/24')")
            .build()
    }
    ...
    fn execute(args: Value) -> Result<CallToolResult, Error> {
        ...
        match name {
            ...
            _ => Ok(ContentBuilder::error(format!("Unknown IP operation: {}", name))),
        }
    }
}
...
fn create_ipv4(args: serde_json::Map<String, Value>) -> Result<CallToolResult, Error> {
    ...
    let octet_values: Result<Vec<u8>, _> = octets.iter()
...
fn create_ipv6(args: serde_json::Map<String, Value>) -> Result<CallToolResult, Error> {
    ...
    let segment_values: Result<Vec<u16>, _> = segments.iter()
        .map(|v| v.as_u64().and_then(|n| if n <= 65535 { Some(n as u16) } else { None }))
        .collect::<Option<Vec<_>>>()
        .ok_or_else(|| Error::msg("All segments must be numbers 0-65535"));

    match segment_values {
        Ok(vals) => {
            let ipv6 = Ipv6Addr::new(vals[0], vals[1], vals[2], vals[3], vals[4], vals[5], vals[6], vals[7]);
            Ok(ContentBuilder::text(json!({
