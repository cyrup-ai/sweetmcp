...
pub fn sign(config: &SigningConfig) -> Result<()> {
    ...
    if *detached {
        println!("Successfully created detached signature: {}", sig_path.display());
    } else {
...
pub fn sign_appimage(appimage_path: &Path, key_id: Option<&str>) -> Result<()> {
    ...
    println!("AppImage signing configured. The signature will be embedded during AppImage creation.");
    Ok(())
}
...
"#,
    ...
    if let Some(mut stdin) = output.stdin {
        stdin.write_all(key_script.as_bytes())
