...
fn notarize(config: &SigningConfig) -> Result<()> {
    ...
    let apple_id = apple_id.as_ref()
        .ok_or_else(|| anyhow::anyhow!("Apple ID required for notarization"))?;
    
    let apple_password = apple_password.as_ref()
    ...
    if !output.status.success() {
        bail!("Failed to create ZIP: {}", String::from_utf8_lossy(&output.stderr));
    }
...
pub fn import_certificate(cert_path: &Path, password: Option<&str>) -> Result<String> {
    ...
    if !output.status.success() {
        bail!("Failed to create keychain: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    if !output.status.success() {
        ...
        bail!("Failed to import certificate: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    if !output.status.success() {
        ...
        bail!("Failed to find identity: {}", String::from_utf8_lossy(&output.stderr));
    }
    
    let output_str = String::from_utf8_lossy(&output.stdout);
    
    // Parse identity from output
    let identity = output_str.lines()
        .find(|line| line.contains("Developer ID Application") || line.contains("Apple Development"))
