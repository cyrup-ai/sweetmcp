...
impl PeerRegistry {
    ...
    pub fn mark_peer_success(&self, addr: &SocketAddr) {
        let mut peers = match self.inner.write() {
            Ok(peers) => peers,
            Err(poisoned) => {
                tracing::warn!("Peer registry write lock poisoned during mark_peer_success, recovering");
                poisoned.into_inner()
            }
        };
    ...
    pub fn mark_peer_failed(&self, addr: &SocketAddr) {
        let mut peers = match self.inner.write() {
            Ok(peers) => peers,
            Err(poisoned) => {
                tracing::warn!("Peer registry write lock poisoned during mark_peer_failed, recovering");
                poisoned.into_inner()
            }
        };
        if let Some(peer) = peers.get_mut(addr) {
    ...
    pub fn get_healthy_peers(&self) -> Vec<SocketAddr> {
        let peers = match self.inner.read() {
            Ok(peers) => peers,
            Err(poisoned) => {
                tracing::warn!("Peer registry read lock poisoned during get_healthy_peers, recovering");
                poisoned.into_inner()
            }
        };
        peers
...
impl ServiceDiscovery for PeerDiscovery {
    fn discover<'life0, 'async_trait>(
        &'life0 self,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(BTreeSet<Backend>, HashMap<u64, bool>)>> + Send + 'async_trait>>
    where
        'life0: 'async_trait,
        Self: 'async_trait,
    {
