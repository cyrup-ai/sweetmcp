...
impl ShutdownCoordinator {
    ...
    async fn deregister_from_discovery(&self) -> Result<()> {
        ...
        if errors.is_empty() {
            ...
            let error_msg = format!("Deregistration completed with {} errors: {}", 
    ...
    async fn send_mdns_goodbye_packets(&self) -> Result<()> {
        ...
        let goodbye_msg = format!("SWEETMCP|{}|{}|GOODBYE", 
            crate::peer_discovery::BUILD_ID, 
            self.local_port
        );
        ...
        for i in 0..MDNS_GOODBYE_REPEATS {
            match socket.send_to(goodbye_msg.as_bytes(), dest).await {
                Ok(_) => debug!("Sent mDNS goodbye packet {}/{}", i + 1, MDNS_GOODBYE_REPEATS),
    ...
    async fn notify_peers_of_shutdown(&self) -> Result<()> {
        ...
        for peer_addr in peers {
            ...
            match timeout(notification_timeout, self.send_shutdown_notification(&peer_addr)).await {
                Ok(Ok(())) => {
        ...
        info!("Peer shutdown notifications: {} successful, {} failed", successful, failed);
    ...
    async fn send_shutdown_notification(&self, peer_addr: &SocketAddr) -> Result<()> {
        ...
        let shutdown_msg = format!("SWEETMCP|{}|SHUTDOWN|{}", 
            crate::peer_discovery::BUILD_ID,
            self.local_port
        );
    ...
    async fn deregister_from_shuttle(&self) -> Result<()> {
        ...
        let service_name = std::env::var("SHUTTLE_SERVICE_NAME")
        ...
        info!("Shuttle deregistration completed for service: {}", service_name);
        Ok(())
    }
