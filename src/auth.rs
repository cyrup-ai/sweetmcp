...
impl JwtAuth {
    ...
    pub fn has_role(&self, claims: &Claims, required_role: &Role) -> bool {
        claims.roles.iter().any(|role| {
            Role::from_str(role).map_or(false, |r| r == *required_role)
        })
    }
    ...
    pub fn has_permission(&self, claims: &Claims, required_permission: &Permission) -> bool {
        claims.permissions.iter().any(|perm| {
            Permission::from_str(perm).map_or(false, |p| p == *required_permission)
        })
    }
    ...
    pub fn has_any_permission(&self, claims: &Claims, required_permissions: &[Permission]) -> bool {
        required_permissions.iter().any(|req_perm| {
            self.has_permission(claims, req_perm)
        })
    }
...
impl AuthContext {
    pub fn from_claims(claims: Claims) -> Self {
        ...
        let permissions: HashSet<Permission> = claims.permissions.iter()
