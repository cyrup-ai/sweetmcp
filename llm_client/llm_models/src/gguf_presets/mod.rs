...
impl GgufPreset {
    pub fn load(self) -> crate::Result<LocalLlmModel> { // Use crate::Result
    ...
    pub fn select_quant_for_available_memory(
    ...
    ) -> crate::Result<u8> { // Use crate::Result
        let ctx_size = ctx_size.unwrap_or(DEFAULT_PRESET_CONTEXT_LENGTH);
        let ctx_memory_size_bytes = estimate_context_size(
            ctx_size,
            self.config.embedding_length,
        ...
        loop {
            ...
            if q_bits == 0 { // Stop if we reach 0 bits without finding a match
                return Err(crate::Error::ModelLoad(format!(
                    "No GGUF file found in preset '{}' matching quantization levels down to 1 bit (started search from {} bits)",
                    self.model_id, initial_q_bits
                )));
            } else {
                q_bits -= 1; // Try the next lower quantization level
            }
        }
    }
