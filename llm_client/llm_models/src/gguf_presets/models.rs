...
pub trait GgufPresetTrait {
    ...
    fn preset_from_str(mut self, selected_model_id: &str) -> crate::Result<Self> // Use crate::Result
    where
        Self: Sized,
    {
        let preset = GgufPreset::all_models()
            ...
            .ok_or_else(|| {
                crate::Error::Config(format!("Invalid GGUF preset model_id: {}", selected_model_id))
            })?;
