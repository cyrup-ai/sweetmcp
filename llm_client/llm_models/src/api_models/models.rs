...
impl ApiLlmModel {
    pub fn model_from_model_id(model_id: &str) -> crate::Result<ApiLlmModel> {
        for provider in ApiLlmProvider::all_providers() {
            if let Ok(preset) = provider.preset_from_model_id(model_id) {
                // Found a matching preset for this provider
                return ApiLlmModel::from_preset(preset); // from_preset already returns Result
            }
        }
        // If no provider had a matching preset
        Err(Error::Config(format!(
            "No API preset found for model_id: {}",
            model_id
        )))
    }
}
