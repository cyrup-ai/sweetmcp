...
use crate::{bail, Error, Result}; // Use local Error/Result/bail
use tokenizers::{
    decoders::{
...
use tracing::warn; // Use warn instead of println
...
impl<'a> GgufTokenizerProps<'a> {
    fn new(ggml: &'a GgmlTokenizerMetadata) -> Result<Self> {
        let props = Self {
...
pub fn convert_gguf_to_hf_tokenizer(ggml: &GgmlTokenizerMetadata) -> Result<Tokenizer> {
    ...
    let tokenizer = match props.model {
        ...
        _ => {
            bail!("Tokenizer model `{:?}` not supported.", props.model);
        }
    };
...
fn get_token(p: &GgufTokenizerProps, token_id: u32) -> Result<String> {
    p.tokens
        .get(token_id as usize)
        .map(ToString::to_string)
        .ok_or_else(|| Error::Tokenizer(format!("Token not found for ID: {}", token_id)))
}

fn unigram_tokenizer(p: &GgufTokenizerProps) -> Result<Tokenizer> {
    ...
    let model = {
        let vocab: Vec<(String, f64)> = if let Some(s) = p.scores.as_ref() {
            ...
            warn!("unigram tokenizer is missing required metadata `tokenizer.ggml.scores`");
            p.tokens.iter().cloned().map(|s| (s, -10000.0)).collect()
        };

        Unigram::from(vocab, Some(unk as usize), true).map_err(|e| Error::HfTokenizer(e.to_string()))?
    };
    ...
    let d = DecoderWrapper::try_from(decoder).map_err(|e| Error::HfTokenizer(e.to_string()))?;
    tokenizer.with_decoder(Some(d));

    let n = NormalizerWrapper::try_from(normalizer).map_err(|e| Error::HfTokenizer(e.to_string()))?;
...
#[allow(dead_code)] // Keep allow dead_code for now, might be useful later
enum Decoder<'a> {
    ByteFallback,
    Fuse,
    Replace(&'a str, &'a str),
...
impl TryFrom<Decoder<'_>> for DecoderWrapper {
    type Error = String; // Match tokenizers crate error type if possible, or use String

    fn try_from(variant: Decoder) -> std::result::Result<Self, Self::Error> {
        let value: DecoderWrapper = match variant {
            ...
            Decoder::Replace(pattern, content) => Replace::new(pattern, content)
                .map_err(|e| e.to_string())? // Ensure .to_string() is used
                .into(),
            ...
            Decoder::Sequence(decoders) => {
                let seq = decoders
                    ...
                    .collect::<std::result::Result<Vec<DecoderWrapper>, String>>()?; // Collect into Result<Vec, String>
...
impl TryFrom<Normalizer<'_>> for NormalizerWrapper {
    ...
    fn try_from(variant: Normalizer) -> std::result::Result<Self, Self::Error> {
        let value: NormalizerWrapper = match variant {
            ...
            Normalizer::Sequence(decoders) => {
                let seq = decoders
                    ...
                    .collect::<std::result::Result<Vec<NormalizerWrapper>, String>>()?; // Collect into Result<Vec, String>
