...
impl GgufLayers {
    ...
    pub fn total_size_bytes(&self) -> usize {
        self.0.values().map(|l| l.size()).sum() // Use values()
    }

    pub fn total_size_blocks_bytes(&self) -> usize {
        self.0
            .values()
            .filter(|l| l.name.starts_with("blk")) // Filter based on layer name if needed
            .map(|l| l.size())
            .sum()
    }
}
...
impl GgufLayer {
    ...
    pub fn size(&self) -> usize {
        self.tensors.values().map(|t| t.size()).sum() // Use values()
    }
}
