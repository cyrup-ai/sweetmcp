...
use crate::requests::completion::CompletionRequestConfig;
...
pub trait RequestConfigTrait {
    fn config(&mut self) -> &mut CompletionRequestConfig;
    ...
    fn presence_penalty(&mut self, presence_penalty: f32) -> &mut Self {
        match presence_penalty {
            value if (-2.0..=2.0).contains(&value) => self.config().presence_penalty = Some(value),
            _ => self.config().presence_penalty = None,
        };
        self
    }
    ...
    fn temperature(&mut self, temperature: f32) -> &mut Self {
        match temperature {
            ...
            _ => self.config().temperature = 0.5, // CompletionRequestConfig uses 0.5 as default
        };
        self
    }
    ...
    /// Sets the number of retry attempts after a failure.
    ...
    /// Sets whether to increase the token limit on failure.
    ...
    /// Sets whether to cache the prompt for subsequent requests.
