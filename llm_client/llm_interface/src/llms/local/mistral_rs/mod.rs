...
use crate::llms::local::mistral_rs::completion::{self, LocalLlmClient};
use crate::requests::{CompletionFinishReason, CompletionResponseChunk, CompletionRequest, CompletionResponse, CompletionError, StoppingSequence};
...
impl MistralRsBackend {
    ...
    /// Handles a streaming completion request for MistralRs backend
    ...
    > {
        ...
        let mut mistral_request = completion::new(request, tx.clone(), id)?;
        
        // Enable streaming in the request
        if let mistralrs::Request::Normal(normal) = &mut mistral_request {
            normal.is_streaming = true;
        }
        ...
        Ok(StreamExt::map(tokio_stream::wrappers::ReceiverStream::new(rx), move |response| {
            match response {
                Response::Chunk(chunk) | Response::CompletionChunk(chunk) => {
                    // Get the text from the chunk
                    let text = chunk.text.clone();
                    
                    Ok(CompletionResponseChunk {
                        id: id.to_string(),
                        index: 0, // MistralRs only supports one generation at a time
                ...
                Response::Done(_) => {
                    // This indicates the stream is done
                    Ok(CompletionResponseChunk {
                        id: id.to_string(),
                        index: 0,
                        text_delta: None,
                        ...
                        tool_call_name: None,
                        tool_call_input_delta: None,
                        finish_reason: Some(CompletionFinishReason::MatchingStoppingSequence(StoppingSequence::InferenceDone("done".to_string()))),
                    })
                },
                ...
                _ => {
                    // Handle other response types as errors
                    Err(CompletionError::LocalClientError(format!("Unexpected response type: {:?}", response)))
                }
            }
        }))
    }
