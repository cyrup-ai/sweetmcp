...
use crate::llms::api::openai::tool_use::{Tool, ToolChoice};
...
pub struct OpenAiCompletionRequest {
    ...
    /// Each function tool specified in the tools array consists of a name, description, and parameters.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Specifying a particular tool like {"type: "function", "function": {"name": "my_function"}}
    /// forces the model to call that tool.
    /// "any" means the model can call any provided tool.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl OpenAiCompletionRequest {
    pub fn new(req: &CompletionRequest) -> crate::Result<Self, CompletionError> {
        ...
        let tools = req.config.tools.as_ref().map(|tools| {
            tools.iter()
                .map(|tool_def| Tool::from(tool_def.clone()))
                .collect::<Vec<_>>()
        }).filter(|tools| !tools.is_empty());

        // Convert tool choice if present
        let tool_choice = req.config.tool_choice.as_ref()
            .map(|choice| ToolChoice::from(choice.clone()));
        ...
        Ok(OpenAiCompletionRequest {
            ...
            tools,
            tool_choice,
            stream: Some(req.config.stream_response),
        })
    }
}
