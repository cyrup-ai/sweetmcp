...
use anyhow::Result;
// use chrono::Duration; // Unused
// use core::panic; // Unused
...
impl LlmBenchmark {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        for mut model_row in self.models.clone() {
            let llm_client_result = match model_row.backend {
                Backend::LlamaCpp => {
                    let mut builder = LlmClient::llama_cpp();
                    if let Some(model_id) = &model_row.model_id {
                        // Assuming LlmPreset::from_model_id exists and returns a GgufPreset or similar
                        // This part might need adjustment based on actual LlmPreset/GgufPreset structure
                        // builder = builder.gguf_preset(LlmPreset::from_model_id(model_id)?);
                        // Placeholder: Assume model_id directly maps to a GgufPreset variant name for now
                        // This needs a proper mapping function based on LlmPreset/GgufPreset definitions
                        match llm_models::gguf_presets::GgufPreset::from_model_id(model_id) {
                            Ok(preset) => builder.llm_loader.gguf_preset_loader.llm_preset = preset,
                            Err(_) => anyhow::bail!("Could not find GGUF preset for model_id: {}", model_id),
                        }

                    } else {
                        anyhow::bail!("Model id not found for LlamaCpp model");
                    };
                    if let Some(ctx_size) = model_row.ctx_size {
                        builder.config.local_config.context_size = Some(ctx_size);
                    };
                    // available_vram is not directly settable on the builder, it's detected.
                    // if let Some(available_vram) = model_row.available_vram {
                    //     builder = builder.available_vram(available_vram);
                    // };
                    if let Some(n_gpu_layers) = model_row.n_gpu_layers {
                         builder.config.local_config.device_config.gpu_layers = Some(n_gpu_layers);
                    };
                    builder.init().await
                }
                Backend::OpenAI => {
                    ...
                    let model_id = model_row.model_id.as_ref()
                        .ok_or_else(|| anyhow::anyhow!("Model id not found for OpenAI model"))?;
                    // Assuming LlmPreset::from_model_id exists and returns an ApiLlmPreset
                    // This needs adjustment based on actual LlmPreset/ApiLlmPreset structure
                    // Placeholder: Assume model_id directly maps to an ApiLlmPreset variant name
                     match llm_models::ApiLlmPreset::from_model_id(model_id) {
                        Ok(preset) => LlmClient::openai().preset(preset).init(),
                        Err(_) => anyhow::bail!("Could not find OpenAI preset for model_id: {}", model_id),
                    }
                }
                Backend::Anthropic => {
                    if let Some(parse_method) = &self.parse_method {
                         match parse_method {
                            ParserMethod::Grammar | ParserMethod::LogitBias => {
                                eprintln!("{:?} decision backend not supported for Anthropic models", parse_method);
                                continue; // Skip this model
                            }
                            ParserMethod::Basic => {} // Basic is fine
                        }
                    }
                     let model_id = model_row.model_id.as_ref()
                        .ok_or_else(|| anyhow::anyhow!("Model id not found for Anthropic model"))?;
                    // Placeholder: Assume model_id directly maps to an ApiLlmPreset variant name
                     match llm_models::ApiLlmPreset::from_model_id(model_id) {
                        Ok(preset) => LlmClient::anthropic().preset(preset).init(),
                        Err(_) => anyhow::bail!("Could not find Anthropic preset for model_id: {}", model_id),
                    }
                }
            };
            ...
            match llm_client_result {
                Ok(llm_client) => {
                    // TODO: Update how default_request_config is handled. It's part of CompletionRequest now.
                    // llm_client.default_request_config = self.default_request_config.clone();

                    // TODO: Update how model_url is retrieved. It might be part of the model struct within the backend.
                    // model_row.model_url = Some(llm_client.backend.get_model_url());

                    // Run benchmark (this function also needs updating)
                    let benchmark_model_id = self.run_model_benchmark(&llm_client).await?;
                    model_row.model_id = Some(benchmark_model_id); // Update model_id with the one used/confirmed by benchmark
                    benchmarked_models.push(model_row);
                }
                Err(e) => {
                     eprintln!("Failed to initialize LLM client for model {:?}: {}", model_row.model_id, e);
                     // Optionally push a row with error status or skip
                     continue;
                }
            }
        }
    ...
    fn create_final_results(&mut self) -> Result<()> {
        for model_row in &self.models {
            let model_id = if let Some(id) = &model_row.model_id {
                id.clone()
            } else {
                 // This should ideally not happen if initialization succeeded and updated the model_id
                 eprintln!("Warning: model_id missing for a model that should have been benchmarked. Skipping final results for this entry.");
                 continue;
            };
            let path = format!(
                "{}/{}.csv",
            ...
            if !std::path::Path::new(&path).exists() {
                ...
                continue;
                 ...
                 match self.create_model_benchmark_result(model_row, path) {
                    Ok(result) => self.bechmark_results.push(result),
                    Err(e) => eprintln!("Failed to create benchmark result for {}: {}", model_id, e),
                }
            }
        }
        Ok(())
