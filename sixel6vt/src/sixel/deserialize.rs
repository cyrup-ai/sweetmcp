...
/// RGB color used in sixel images
...
impl From<[u16; 3]> for SixelColor {
    fn from(color_coordinate_system: [u16; 3]) -> Self {
        let [r, g, b] = color_coordinate_system;
        // Scale the 0-100 range to 0-255
        let r = (r as f32 * 255.0 / 100.0).round() as u8;
        let g = (g as f32 * 255.0 / 100.0).round() as u8;
        let b = (b as f32 * 255.0 / 100.0).round() as u8;
        SixelColor { r, g, b }
    }
}

// Also implement From<sixel_tokenizer::ColorCoordinateSystem>
impl From<sixel_tokenizer::ColorCoordinateSystem> for SixelColor {
    fn from(color_coordinate_system: sixel_tokenizer::ColorCoordinateSystem) -> Self {
...
impl SixelDeserializer {
    ...
    fn add_sixel_byte(&mut self, byte: u8, repeat_count: usize) {
        for (pixel_line_index_in_sixel, bit) in
            SixelPixelIterator::new(byte.saturating_sub(63)).enumerate()
        {
    ...
    fn pad_lines_vertically(&mut self, pad_until: usize) {
        ...
        if self.pixels.len() < pad_until {
            ...
            let line_padding = std::iter::repeat_n(empty_line, lines_to_pad);
    ...
    fn pad_lines_horizontally(&mut self, pad_until: usize) {
        ...
        for pixel_line in self.pixels.iter_mut() {
            if pixel_line.len() < pad_until {
                ...
                let pixel_padding = std::iter::repeat_n(empty_pixel, pixel_count_to_pad);
