...
pub fn prompts_list_stream(plugin_manager: PluginManager) -> crate::prompt::model::PromptStream {
    ...
    tokio::spawn(async move {
        let prompt_info_map = plugin_manager.prompt_info.read().await;
        for (_, metadata) in prompt_info_map.values() {
...
pub fn prompts_get_pending(
    ...
    tokio::spawn(async move {
        ...
        let (plugin_name, prompt_metadata) = {
            let prompt_info_map = plugin_manager.prompt_info.read().await;
            // Find by id
            let found = prompt_info_map
                .values()
                .find(|(_, prompt)| prompt.id == prompt_id);
            match found {
                Some((name, metadata)) => (name.clone(), metadata.clone()),
                None => {
        ...
        {
            let mut plugins_map = plugin_manager.plugins.write().await;
            let plugin = match plugins_map.get_mut(&plugin_name) {
                Some(plugin) => plugin,
                None => {
                    let _ = tx.send(Err(HandlerError::new(format!(
            ...
            template_content = match plugin.call::<Json<serde_json::Value>, String>(
