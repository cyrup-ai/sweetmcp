...
EOF
    ...
    fn generate_plist(b: &InstallerBuilder) -> String {
        ...
        if !b.env.is_empty() {
            let env_dict: HashMap<String, Value> = b.env.iter()
                .map(|(k, v)| (k.clone(), Value::String(v.clone())))
                .collect();
            plist.insert("EnvironmentVariables".to_string(), 
        ...
        if b.run_as_group != "wheel" && b.run_as_group != "staff" {
            plist.insert("GroupName".to_string(), Value::String(b.run_as_group.clone()));
        }
        
        // Auto-restart
        plist.insert("KeepAlive".to_string(), 
            if b.auto_restart {
        ...
        // Logging
        plist.insert("StandardOutPath".to_string(), 
                    Value::String(format!("/var/log/{}/stdout.log", b.label)));
        plist.insert("StandardErrorPath".to_string(), 
        ...
        // Network dependency
        if b.wants_network {
            plist.insert("LimitLoadToSessionType".to_string(), 
