...
pub fn speak_tool() -> Tool {
    ...
    properties.insert("text".to_string(), ToolInputSchemaProperty {
        type_name: Some("string".to_string()),
        enum_values: None,
        description: Some("Text to convert to speech".to_string()),
    });
    
    properties.insert("voice_id".to_string(), ToolInputSchemaProperty {
        type_name: Some("string".to_string()),
        enum_values: None,
        description: Some("Voice ID to use (optional, defaults to system voice)".to_string()),
    });
    
    properties.insert("speed".to_string(), ToolInputSchemaProperty {
...
pub fn listen_tool() -> Tool {
    ...
    properties.insert("microphone_id".to_string(), ToolInputSchemaProperty {
        type_name: Some("string".to_string()),
        enum_values: None,
        description: Some("Microphone device to use (e.g., 'default', 'USB Microphone')".to_string()),
    });
    
    properties.insert("duration_seconds".to_string(), ToolInputSchemaProperty {
        type_name: Some("integer".to_string()),
        enum_values: None,
        description: Some("How long to listen in seconds (1-300)".to_string()),
    });
    
    properties.insert("wake_word".to_string(), ToolInputSchemaProperty {
        type_name: Some("string".to_string()),
        enum_values: None,
        description: Some("Optional wake word to listen for (e.g., 'hey assistant')".to_string()),
    });
