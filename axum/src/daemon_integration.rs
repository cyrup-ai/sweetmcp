...
use cyrupd::{
    ServiceConfig, ServiceDefinition, ServiceManager, 
    daemonise, HealthCheckConfig, LogRotationConfig
};
use crate::plugin::manager::PluginManager;
use crate::ui::ServeArgs;

/// Integrate MCP server with the daemon manager
pub fn run_mcp_as_daemon(
...
pub async fn run_mcp_server_standalone(
    ...
    info!("Starting MCP server in standalone mode on socket: {:?}", socket_path);
