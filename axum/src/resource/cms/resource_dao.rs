        ...
        match response {
            Ok(node_rows) => {
                ...
                for row in node_rows {
                    ...
                    // Helper to convert Option<Thing> to Option<Url>
                    let thing_to_url = |thing: Option<surrealdb::sql::Thing>| -> Option<Url> {
                        thing.and_then(|t| Url::parse(&format!("cms://{}", t)).ok())
                    };

                    // Convert content to a string if present - using it to address warning
                    ...
                    // Use category for resource node_type if available, otherwise use type_
                    let node_type = match &row.category {
                        Some(category) => format!("{}/{}", row.type_, category),
                        None => row.type_
                    };
                    ...
                    let uri = match Url::parse(&format!("cms://{}", row.id)) {
                        Ok(url) => url,
                        Err(e) => {
                            log::error!("Failed to parse resource URI: {}", e);
                            // Fallback to a basic URL
                            Url::parse(&format!("cms://resource/{}", row.id.id)).unwrap_or_else(|_| {
                    ...
                    let vec_thing_to_vec_url = |things: Option<Vec<surrealdb::sql::Thing>>| -> Option<Vec<Url>> {
                        things.map(|ts| {
                            ts.into_iter()
                                .filter_map(|t| Url::parse(&format!("cms://{}", t)).ok())
                                .collect()
                        })
                    };
...
pub fn find_by_slug(slug: &str) -> AsyncResource {
    match get_resource_dao() {
        Ok(dao) => {
            // Construct the Thing ID string (e.g., "node:slug_value")
            let thing_id_str = format!("node:{}", slug);
            // Attempt to parse the Thing ID string into a URL representing the resource URI
            // This assumes a direct mapping; adjust if your URI scheme is different
            match Url::parse(&format!("cms://{}", thing_id_str)) {
