...
pub use rpc::{GetContextRequest, GetContextResult, ContextItem, ContextContent, SubscribeContextRequest, SubscribeContextResult, ContextChangedNotification, context_get, context_subscribe};
...
impl ApplicationContext {
    pub fn initialize(
        ...
        crate::types::AsyncTask::from_future(async move {
            ...
            // Parse configuration
            let config: crate::config::Config = crate::config::parse_config_from_str(&config_content)
            ...
            if let Some(app_db_config) = &config.database {
                ...
                let url = app_db_config.url.as_ref().ok_or_else(|| anyhow::anyhow!("Database URL missing"))?;
    ...
    pub fn logger(&self) -> Arc<logger::ConsoleLogger> { // Use submodule path
        self.logger.clone()
    }
...
pub async fn initialize_global_context(
    ...
    let app_context = APPLICATION_CONTEXT.get()
