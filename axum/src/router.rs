use std::{collections::HashMap, sync::Arc, fs, os::unix::fs::PermissionsExt};
...
fn build_rpc_router(
    ...
    // Create a PluginManager to share with handlers
...
pub async fn run_server(config: Config, plugin_manager: PluginManager, serve_args: ServeArgs) -> Result<()> {
    // Initialize memory system if configured
    if let Some(db_config) = &config.database {
        let memory_config = surreal_memory::MemoryConfig {
            database: surreal_memory::utils::config::DatabaseConfig {
                db_type: surreal_memory::utils::config::DatabaseType::SurrealDB,
                connection_string: db_config.path.clone().map(|p| format!("surrealkv://{}", p)).unwrap_or_else(|| "surrealkv://./data/mcp_memory.db".to_string()),
                namespace: db_config.namespace.clone().unwrap_or_else(|| "mcp".to_string()),
                database: db_config.database.clone().unwrap_or_else(|| "agent_memory".to_string()),
                username: db_config.username.clone(),
                password: db_config.password.clone(),
                pool_size: Some(10),
                options: None,
            },
            vector_store: surreal_memory::utils::config::VectorStoreConfig {
                store_type: surreal_memory::utils::config::VectorStoreType::SurrealDB,
                embedding_model: surreal_memory::utils::config::EmbeddingModelConfig {
                    model_type: surreal_memory::utils::config::EmbeddingModelType::Custom,
            ...
            llm: surreal_memory::utils::config::LLMConfig {
                provider: surreal_memory::utils::config::LLMProvider::Custom,
            ...
            cache: surreal_memory::utils::config::CacheConfig {
                enabled: true,
                cache_type: surreal_memory::utils::config::CacheType::Memory,
                size: Some(10000),
                ttl: Some(3600),
                options: None,
            },
            logging: surreal_memory::utils::config::LoggingConfig {
                level: surreal_memory::utils::config::LogLevel::Info,
        ...
        match surreal_memory::initialize(&memory_config).await {
            Ok(_) => {
...
async fn run_stdio_server(plugin_manager: PluginManager) -> Result<()> {
    ...
    // Set up the router with plugin manager
    let plugins = plugin_manager.plugins;
    let tool_to_plugin = Arc::new(RwLock::new(HashMap::new()));

    // Build RPC router with all handlers
    let rpc_router = build_rpc_router(plugins, tool_to_plugin);
...
async fn run_daemon(plugin_manager: PluginManager, serve_args: ServeArgs) -> Result<()> {
    info!("Starting MCP JSON-RPC server with daemon manager");
    
    // Use our sophisticated daemon manager
    crate::daemon_integration::run_mcp_as_daemon(plugin_manager, serve_args)
}
