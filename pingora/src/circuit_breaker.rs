...
IntGauge::new("sweetmcp_circuit_breaker_state_fallback", "Fallback circuit breaker state gauge")
            .unwrap_or_else(|e| {
...
IntCounter::new("sweetmcp_circuit_breaker_opened_total_fallback", "Fallback circuit breaker opened counter")
            .unwrap_or_else(|e| {
    ...
    async fn transition_to_half_open(&self) {
        ...
        if *state != CircuitState::HalfOpen {
            *state = CircuitState::HalfOpen;
            self.half_open_remaining.store(self.config.half_open_requests, Ordering::SeqCst);
...
impl CircuitBreakerManager {
    ...
    pub async fn get_all_breakers(&self) -> Vec<(String, Arc<CircuitBreaker>)> {
        let breakers = self.breakers.read().await;
        breakers.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
    }
}
