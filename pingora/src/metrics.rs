...
pub fn record_discovery(operation: &str, success: bool, duration_secs: f64) {
    ...
    DISCOVERY_COUNTER.with_label_values(&[operation, status]).inc();
    DISCOVERY_LATENCY.with_label_values(&[operation]).observe(duration_secs);
}
...
pub fn record_circuit_breaker_state(peer: &str, state: &str) {
    CIRCUIT_BREAKER_STATE.with_label_values(&[peer, state]).inc();
}
...
pub fn increment_active_requests(method: &str, endpoint: &str) {
    HTTP_REQUESTS_ACTIVE.with_label_values(&[method, endpoint]).inc();
...
pub fn decrement_active_requests(method: &str, endpoint: &str) {
    HTTP_REQUESTS_ACTIVE.with_label_values(&[method, endpoint]).dec();
