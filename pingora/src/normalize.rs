...
fn graphql_to_json_rpc(query_str: &str, doc: ExecutableDocument) -> Result<(ProtocolContext, Value)> {
    // Find the first operation (query or mutation)
    let operation = doc.operations
        .iter()
        .next()
        .ok_or_else(|| anyhow::anyhow!("No GraphQL operation found"))?;
    
    let (method, params) = match &operation.1.node {
        OperationDefinition::Query(query) => {
            extract_graphql_method_and_params(&query.selection_set.node)?
        }
        OperationDefinition::Mutation(mutation) => {
            extract_graphql_method_and_params(&mutation.selection_set.node)?
        }
...
fn extract_graphql_method_and_params(selection_set: &[Selection]) -> Result<(String, Value)> {
    ...
    if !field.selection_set.node.is_empty() {
        let subfield = match &field.selection_set.node[0].node {
...
fn extract_graphql_arguments(arguments: &[(Positioned<Name>, Positioned<Value>)]) -> Value {
...
fn graphql_value_to_json(value: &Value) -> serde_json::Value {
    match value {
        ...
        Value::List(list) => {
            let items: Vec<_> = list.iter()
                .map(|v| graphql_value_to_json(&v.node))
                .collect();
            json!(items)
        }
        Value::Object(obj) => {
            ...
            for (key, val) in obj {
                map.insert(key.node.as_str().to_string(), graphql_value_to_json(&val.node));
            }
            json!(map)
        }
        Value::Variable(_) => {
...
fn capnp_to_json_rpc(body: &[u8]) -> Result<(ProtocolContext, Value)> {
...
pub fn from_json_rpc(ctx: &ProtocolContext, json_rpc_response: &Value) -> Result<Vec<u8>> {
    match ctx.protocol {
        Proto::JsonRpc => {
        ...
        Proto::GraphQL => {
            graphql_from_json_rpc(ctx, json_rpc_response)
        }
        Proto::Capnp => {
            capnp_from_json_rpc(ctx, json_rpc_response)
        }
    }
}

/// Convert JSON-RPC response to GraphQL response
fn graphql_from_json_rpc(ctx: &ProtocolContext, response: &Value) -> Result<Vec<u8>> {
...
fn capnp_from_json_rpc(_ctx: &ProtocolContext, _response: &Value) -> Result<Vec<u8>> {
