...
impl TokenManager {
    ...
    pub async fn decrypt_token(&self, encrypted: &EncryptedToken) -> Result<String> {
        ...
        if encrypted.key_id == current.key_id {
            if let Ok(plaintext) = sealedbox::open(&ciphertext, &current.public_key, &current.secret_key) {
                drop(current);
        ...
        if let Some(prev_keypair) = previous.as_ref() {
            if encrypted.key_id == prev_keypair.key_id {
                if let Ok(plaintext) = sealedbox::open(&ciphertext, &prev_keypair.public_key, &prev_keypair.secret_key) {
                    drop(previous);
                    return self.extract_token_from_plaintext(&plaintext).await;
                }
            }
    ...
    async fn cleanup_revoked_tokens(&self) {
        ...
        if removed > 0 {
            info!("Cleaned up {} expired revoked tokens, {} remaining", removed, revoked.len());
        } else if before_count > 1000 {
            // Log if we have a large number of active revoked tokens
            info!("Revoked token list size: {} (all still within validity period)", before_count);
        }
    }
