...
use rcgen::{CertificateParams, DistinguishedName, DnType, SanType, Issuer, KeyPair};
use x509_cert::{Certificate as X509CertCert, der::Decode};
...
use super::ocsp::{OcspStatus, OcspCache};
...
impl CrlCache {
    ...
    pub async fn check_certificate_revocation(&self, cert: &ParsedCertificate) -> Result<bool, TlsError> {
        ...
        for crl_url in &cert.crl_urls {
            match self.check_against_crl(cert, crl_url).await {
                Ok(is_revoked) => {
                    if is_revoked {
                        tracing::warn!("Certificate serial {:?} found in CRL from {}", 
                                      hex::encode(&cert.serial_number), crl_url);
                        return Ok(true);
                    }
                    tracing::info!("Certificate serial {:?} not found in CRL from {}", 
                                  hex::encode(&cert.serial_number), crl_url);
                }
                Err(e) => {
    ...
    async fn check_against_crl(&self, cert: &ParsedCertificate, crl_url: &str) -> Result<bool, TlsError> {
    ...
    fn is_crl_cache_expired(entry: &CrlCacheEntry) -> bool {
        ...
        now.duration_since(entry.cached_at).unwrap_or(Duration::ZERO) > cache_duration
    }

    #[inline]
    fn cache_crl(&self, cache_key: String, entry: CrlCacheEntry) {
        match self.cache.write() {
            Ok(mut cache) => { cache.insert(cache_key, entry); },
            Err(poisoned) => {
    ...
    async fn download_and_parse_crl(&self, crl_url: &str) -> Result<CrlCacheEntry, TlsError> {
        ...
        if !response.status().is_success() {
            return Err(TlsError::CrlValidation(format!(
                "CRL download failed with status: {}", response.status()
            )));
        }
    ...
    fn parse_crl_data(&self, crl_bytes: &[u8]) -> Result<CrlCacheEntry, TlsError> {
        ...
        let der_bytes = if crl_bytes.starts_with(b"-----BEGIN") {
            ...
            if der_data.is_empty() {
                return Err(TlsError::CrlValidation("No CRL data found in PEM".to_string()));
            }
        ...
        let next_update = crl.next_update()
            .map(|time| std::time::UNIX_EPOCH + std::time::Duration::from_secs(time.timestamp() as u64));

        tracing::info!("Parsed CRL with {} revoked certificates, next update: {:?}", 
                      revoked_serials.len(), next_update);
...
impl TlsManager {
    ...
    pub fn validate_basic_constraints(parsed_cert: &ParsedCertificate, expected_ca: bool) -> Result<(), TlsError> {
        Self::validate_basic_constraints_internal(parsed_cert, expected_ca)
    }
    
    /// Validate KeyUsage extension
    pub fn validate_key_usage(parsed_cert: &ParsedCertificate, usage: CertificateUsage) -> Result<(), TlsError> {
        Self::validate_key_usage_internal(parsed_cert, usage)
    }
    /// Validate certificate using OCSP (Online Certificate Status Protocol)
    pub async fn validate_certificate_ocsp(&self, cert_pem: &str, issuer_cert_pem: Option<&str>) -> Result<(), TlsError> {
        ...
        match self.ocsp_cache.check_certificate(&parsed_cert, issuer_cert.as_ref()).await {
            Ok(OcspStatus::Good) => {
                tracing::info!("OCSP validation successful: certificate is valid");
                Ok(())
            }
            Ok(OcspStatus::Revoked) => {
                Err(TlsError::OcspValidation("Certificate has been revoked".to_string()))
            }
    ...
    pub async fn validate_certificate_crl(&self, cert_pem: &str) -> Result<(), TlsError> {
        ...
        match self.crl_cache.check_certificate_revocation(&parsed_cert).await {
            Ok(false) => {
                tracing::info!("CRL validation successful: certificate is not revoked");
                Ok(())
            }
            Ok(true) => {
                Err(TlsError::CrlValidation("Certificate has been revoked according to CRL".to_string()))
            }
            Err(e) => {
    ...
    pub async fn validate_certificate_chain(&self, cert_chain_pem: &str) -> Result<(), TlsError> {
        ...
        for cert_der in rustls_pemfile::certs(&mut cursor) {
            let cert_der = cert_der.map_err(|e| TlsError::CertificateParsing(format!("Failed to parse certificate from chain: {}", e)))?;
            certificates.push(cert_der);
        }
        
        if certificates.is_empty() {
            return Err(TlsError::ChainValidation("No certificates found in chain".to_string()));
        }
        
        tracing::info!("Validating certificate chain with {} certificates", certificates.len());
        ...
        let ca_trust_anchor = webpki::TrustAnchor::try_from_cert_der(&self.ca_cert)
            .map_err(|e| TlsError::ChainValidation(format!("Failed to create trust anchor from CA: {:?}", e)))?;
        ...
        if !system_roots.is_empty() {
            tracing::debug!("Loaded {} total trust anchors (including system roots)", trust_anchors.len());
        }
        ...
        let end_entity_cert = webpki::EndEntityCert::try_from(end_entity_der.as_ref())
            .map_err(|e| TlsError::ChainValidation(format!("Failed to parse end-entity certificate: {:?}", e)))?;
        ...
        .map_err(|e| TlsError::ChainValidation(format!("Certificate chain validation failed: {:?}", e)))?;
        ...
        for (i, cert_der) in certificates.iter().enumerate() {
            ...
            Self::validate_certificate_time_internal(&parsed_cert)
                .map_err(|e| TlsError::ChainValidation(format!("Certificate {} in chain: {}", i, e)))?;
            
            // For intermediate and root CAs, verify they have proper CA constraints
            if i > 0 {
                Self::validate_basic_constraints_internal(&parsed_cert, true)
                    .map_err(|e| TlsError::ChainValidation(format!("Certificate {} in chain: {}", i, e)))?;
        ...
        tracing::info!("Certificate chain validation successful: chain properly links to trusted root CA");
        Ok(())
    }
    ...
    fn load_system_root_certificates() -> Result<Vec<CertificateDer<'static>>, TlsError> {
        ...
        for path in &possible_paths {
            if let Ok(contents) = std::fs::read_to_string(path) {
                ...
                tracing::debug!("Loaded {} system root certificates from {}", roots.len(), path);
                break;
            }
        }
    ...
    fn validate_encryption_passphrase() -> Result<String, TlsError> {
        ...
        let passphrase = env::var("SWEETMCP_KEY_ENCRYPTION_PASSPHRASE")
            .map_err(|_| TlsError::KeyProtection(
        ...
        if Self::has_weak_patterns(&passphrase) {
            return Err(TlsError::KeyProtection(
                "Encryption passphrase contains weak patterns (sequential or repeated characters)".to_string()
            ));
        }
    ...
    async fn decrypt_private_key(encrypted_data: &[u8]) -> Result<SecureKeyMaterial, TlsError> {
        ...
        if encrypted_data.len() < 60 {
            return Err(TlsError::KeyProtection("Invalid encrypted data format".to_string()));
        }
        ...
        let decrypted = key.open_in_place(nonce, aead::Aad::empty(), &mut ciphertext_copy)
    ...
    fn validate_certificate_time_internal(parsed_cert: &ParsedCertificate) -> Result<(), TlsError> {
        ...
        if let Ok(duration_until_expiry) = parsed_cert.not_after.duration_since(now) {
            if duration_until_expiry.as_secs() < 30 * 24 * 3600 { // 30 days
                tracing::warn!(
    ...
    fn validate_basic_constraints_internal(parsed_cert: &ParsedCertificate, expected_ca: bool) -> Result<(), TlsError> {
        if parsed_cert.is_ca != expected_ca {
            if expected_ca {
                ...
                return Err(TlsError::CertificateValidation(
                    "End-entity certificate incorrectly marked as CA (BasicConstraints CA=true)".to_string()
                ));
            }
        }
    ...
    fn validate_key_usage_internal(parsed_cert: &ParsedCertificate, usage: CertificateUsage) -> Result<(), TlsError> {
        match usage {
            ...
            CertificateUsage::ServerAuth => {
                ...
                if !parsed_cert.key_usage.contains(&"digitalSignature".to_string()) {
                    return Err(TlsError::CertificateValidation(
                ...
                if !parsed_cert.key_usage.contains(&"keyEncipherment".to_string()) {
    ...
    pub fn verify_hostname(parsed_cert: &ParsedCertificate, hostname: &str) -> Result<(), TlsError> {
        ...
        if let Ok(ip_addr) = hostname.parse::<std::net::IpAddr>() {
            ...
            return Err(TlsError::PeerVerification(format!(
                "IP address {} not found in certificate SANs", hostname
            )));
        }
        ...
        Err(TlsError::PeerVerification(format!(
            "Hostname {} does not match any certificate SANs or Common Name", hostname
        )))
    }
    ...
    pub fn verify_peer_certificate(cert_pem: &str, expected_hostname: &str) -> Result<(), TlsError> {
        ...
        tracing::info!("Successfully verified peer certificate for hostname: {}", expected_hostname);
        Ok(())
    }

    /// Verify peer certificate with OCSP validation
    pub async fn verify_peer_certificate_with_ocsp(&self, cert_pem: &str, expected_hostname: &str, issuer_cert_pem: Option<&str>) -> Result<(), TlsError> {
        // Perform standard certificate validation
        Self::verify_peer_certificate(cert_pem, expected_hostname)?;
        
        // Additional OCSP validation
        self.validate_certificate_ocsp(cert_pem, issuer_cert_pem).await?;
        
        tracing::info!("Successfully verified peer certificate with OCSP for hostname: {}", expected_hostname);
        Ok(())
    }

    /// Verify peer certificate with comprehensive revocation checking (OCSP + CRL + Chain)
    pub async fn verify_peer_certificate_comprehensive(&self, cert_pem: &str, expected_hostname: &str, full_chain_pem: Option<&str>) -> Result<(), TlsError> {
        // Perform standard certificate validation
        Self::verify_peer_certificate(cert_pem, expected_hostname)?;
        ...
        self.validate_certificate_ocsp(cert_pem, issuer_cert_pem.as_deref()).await?;
    ...
    fn extract_certificate_details(cert: &X509CertCert) -> Result<(Vec<String>, Vec<std::net::IpAddr>, bool, Vec<String>, SystemTime, SystemTime), TlsError> {
                                                ...
                                                match name_header.tag.number() {
                                                    TagNumber::N2 => {
                                                        ...
                                                        if let Ok(dns_header) = reader.peek_header() {
                                                            if let Ok(dns_bytes) = reader.read_vec(dns_header.length) {
                                                                if let Ok(dns_name) = std::str::from_utf8(&dns_bytes) {
                                                    ...
                                                    TagNumber::N7 => {
                                                        ...
                                                        if let Ok(ip_header) = reader.peek_header() {
                                                            if let Ok(ip_bytes) = reader.read_vec(ip_header.length) {
                                                                // IPv4 = 4 bytes, IPv6 = 16 bytes
                                                                match ip_bytes.len() {
                                                                    4 => {
                                                                        let octets: [u8; 4] = match ip_bytes.try_into() {
                                                                            Ok(octets) => octets,
                                                                            Err(_) => {
                    ...
                    OID_BASIC_CONSTRAINTS => {
                        ...
                        if ext_data.len() >= 3 {
                            for i in 0..ext_data.len()-2 {
                                if ext_data[i] == 0x01 && ext_data[i+1] == 0x01 && ext_data[i+2] == 0xFF {
                                    is_ca = true;
                                    break;
                                }
                            }
                        ...
                        for i in 0..ext_data.len() {
                            if ext_data[i] == 0x03 && i + 2 < ext_data.len() {
                                ...
                                if i + 3 < ext_data.len() {
                                    ...
                                    if bits & 0x80 != 0 { key_usage.push("digitalSignature".to_string()); }
                                    if bits & 0x40 != 0 { key_usage.push("contentCommitment".to_string()); }
                                    if bits & 0x20 != 0 { key_usage.push("keyEncipherment".to_string()); }
                                    if bits & 0x10 != 0 { key_usage.push("dataEncipherment".to_string()); }
                                    if bits & 0x08 != 0 { key_usage.push("keyAgreement".to_string()); }
                                    if bits & 0x04 != 0 { key_usage.push("keyCertSign".to_string()); }
                                    if bits & 0x02 != 0 { key_usage.push("cRLSign".to_string()); }
                                    
                                    // Check second byte if present for last two bits
                                    if i + 4 < ext_data.len() && ext_data[i + 1] > 1 {
                                        let bits2 = ext_data[i + 4];
                                        if bits2 & 0x80 != 0 { key_usage.push("encipherOnly".to_string()); }
                                        if bits2 & 0x40 != 0 { key_usage.push("decipherOnly".to_string()); }
                                    }
                                }
                                break;
                            }
        ...
        Ok((san_dns_names, san_ip_addresses, is_ca, key_usage, not_before, not_after))
    }
    ...
    fn parse_certificate_from_pem_internal(pem_data: &str) -> Result<ParsedCertificate, TlsError> {
        ...
        let subject_der = cert.tbs_certificate.subject.to_der()
            .map_err(|e| TlsError::CertificateParsing(format!("Failed to encode subject DER: {}", e)))?;
        
        // Extract public key DER from TBS certificate
        let public_key_der = cert.tbs_certificate.subject_public_key_info.to_der()
            .map_err(|e| TlsError::CertificateParsing(format!("Failed to encode public key DER: {}", e)))?;
    ...
    async fn generate_ca(cert_dir: &Path) -> Result<(CertificateDer<'static>, PrivatePkcs8KeyDer<'static>, Issuer<'static, KeyPair>)> {
    ...
    async fn load_ca(cert_dir: &Path) -> Result<(CertificateDer<'static>, PrivatePkcs8KeyDer<'static>, Issuer<'static, KeyPair>)> {
        ...
        let mut params = CertificateParams::new(parsed_cert.san_dns_names.iter().map(|s| s.as_str().try_into()).collect::<Result<Vec<_>, _>>()?)?;
    ...
    async fn generate_server_cert(
        ...
        if let Ok(hostname) = hostname::get() {
            if let Some(hostname_str) = hostname.to_str() {
                params.subject_alt_names.push(SanType::DnsName(hostname_str.try_into()?));
            }
        }
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = fs::metadata(cert_dir.join("server.key")).await?.permissions();
            perms.set_mode(0o600);
