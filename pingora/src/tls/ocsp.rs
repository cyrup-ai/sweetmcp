...
use x509_ocsp::{OcspRequest, OcspResponse, CertId};
...
impl OcspCache {
    ...
    pub async fn check_certificate(&self, cert: &ParsedCertificate, issuer_cert: Option<&ParsedCertificate>) -> Result<OcspStatus, TlsError> {
        ...
        if let Some(cached) = self.get_cached_status(&cache_key) {
            if !Self::is_cache_expired(&cached) {
                tracing::debug!("OCSP cache hit for certificate serial: {:?}", hex::encode(&cert.serial_number));
    ...
    async fn perform_ocsp_check(&self, cert: &ParsedCertificate, issuer_cert: Option<&ParsedCertificate>) -> Result<(OcspStatus, Option<SystemTime>), TlsError> {
        ...
        for ocsp_url in &cert.ocsp_urls {
            match self.query_ocsp_responder(cert, issuer, ocsp_url).await {
                Ok(result) => {
                    tracing::info!("OCSP validation successful for certificate serial: {:?}, status: {:?}", 
                                  hex::encode(&cert.serial_number), result.0);
                    return Ok(result);
                }
                Err(e) => {
        ...
        tracing::warn!("All OCSP URLs failed for certificate serial: {:?}, treating as unknown", 
                      hex::encode(&cert.serial_number));
        Ok((OcspStatus::Unknown, None))
    }

    async fn query_ocsp_responder(&self, cert: &ParsedCertificate, issuer: &ParsedCertificate, ocsp_url: &str) -> Result<(OcspStatus, Option<SystemTime>), TlsError> {
        // Create OCSP request
        ...
        if !response.status().is_success() {
            return Err(TlsError::OcspValidation(format!(
                "OCSP responder returned error status: {}", response.status()
            )));
        }
    ...
    fn create_ocsp_request(&self, cert: &ParsedCertificate, issuer_cert: &ParsedCertificate) -> Result<(Vec<u8>, Vec<u8>), TlsError> {
        ...
        let cert_id = CertId {
            ...
            issuer_name_hash: der::asn1::OctetString::new(issuer_name_hash.as_ref())
                .map_err(|e| TlsError::OcspValidation(format!("Failed to create issuer name hash: {}", e)))?,
            issuer_key_hash: der::asn1::OctetString::new(issuer_key_hash.as_ref())
                .map_err(|e| TlsError::OcspValidation(format!("Failed to create issuer key hash: {}", e)))?,
        ...
        let der_bytes = request.to_der()
            .map_err(|e| TlsError::OcspValidation(format!("Failed to encode OCSP request: {}", e)))?;
        
        Ok((der_bytes, nonce))
    }

    fn parse_ocsp_response(&self, response_bytes: &[u8], expected_nonce: &[u8], cert_serial: &[u8]) -> Result<(OcspStatus, Option<SystemTime>), TlsError> {
        let response = OcspResponse::from_der(response_bytes)
            .map_err(|e| TlsError::OcspValidation(format!("Failed to decode OCSP response: {}", e)))?;
        
        // Check response status
        if response.response_status != x509_ocsp::OcspResponseStatus::Successful {
            return Err(TlsError::OcspValidation(format!(
                "OCSP response status: {:?}", response.response_status
            )));
        }
        
        let response_bytes = response.response_bytes
            .as_ref()
            .ok_or_else(|| TlsError::OcspValidation("No response bytes in OCSP response".to_string()))?;
        
        let basic_response = x509_ocsp::BasicOcspResponse::from_der(response_bytes.response.as_bytes())
            .map_err(|e| TlsError::OcspValidation(format!("Failed to parse basic OCSP response: {}", e)))?;
        
        // Verify nonce matches
        if let Some(nonce_ext) = basic_response.tbs_response_data.response_extensions.as_ref()
            .and_then(|exts| exts.iter().find(|ext| ext.extn_id == der::asn1::ObjectIdentifier::new_unwrap("1.3.6.1.5.5.7.48.1.2"))) {
            if nonce_ext.extn_value.as_bytes() != expected_nonce {
                return Err(TlsError::OcspValidation("OCSP nonce mismatch - possible replay attack".to_string()));
            }
        }
        ...
        let single_response = basic_response.tbs_response_data.responses
            ...
            .ok_or_else(|| TlsError::OcspValidation("Certificate not found in OCSP response".to_string()))?;
        ...
        let next_update = single_response.next_update
            .as_ref()
            .and_then(|time| {
    ...
    pub fn cleanup_cache(&self) {
        ...
        tracing::debug!("OCSP cache cleanup completed, {} entries remaining", cache.len());
    }
}