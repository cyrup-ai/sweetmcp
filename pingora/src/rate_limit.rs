...
impl TokenBucket {
    ...
    pub fn try_consume(&mut self, tokens: u32) -> bool {
        ...
        if self.tokens >= tokens as f64 {
            ...
            debug!("Token consumption failed: {} tokens requested, {} available", tokens, self.tokens);
            false
        }
    }
    ...
    fn refill(&mut self) {
        ...
        if elapsed > 0.0 {
            ...
            if new_tokens > 0.1 {
                debug!("Refilled {:.2} tokens, total: {:.2}/{}", new_tokens, self.tokens, self.capacity);
            }
        }
    }
    ...
    pub fn update_config(&mut self, config: &TokenBucketConfig) {
        ...
        info!("Updated token bucket config: capacity={}, refill_rate={}", 
...
impl SlidingWindow {
    ...
    pub fn try_request(&mut self) -> bool {
        ...
        if total_requests < self.max_requests {
            self.add_request();
            debug!("Request allowed: {}/{} in sliding window", total_requests + 1, self.max_requests);
            true
        } else {
            debug!("Request denied: {}/{} in sliding window", total_requests, self.max_requests);
            false
        }
    }
    ...
    pub fn update_config(&mut self, config: &SlidingWindowConfig) {
        ...
        if self.sub_windows.len() != config.sub_windows as usize {
            self.sub_windows.resize(config.sub_windows as usize, (Instant::now(), 0));
            self.sub_window_duration = self.window_size / config.sub_windows;
            self.current_sub_window = 0;
        }
        
        info!("Updated sliding window config: window_size={}s, max_requests={}, sub_windows={}", 
    ...
    pub fn new(algorithm: &RateLimitAlgorithm) -> Self {
        match algorithm {
            RateLimitAlgorithm::TokenBucket(config) => {
                Self::TokenBucket(TokenBucket::new(config))
            }
...
impl AdvancedRateLimitManager {
    pub fn new() -> Self {
        ...
        endpoint_configs.insert("/api/peers".to_string(), EndpointRateConfig {
        ...
        endpoint_configs.insert("/api/register".to_string(), EndpointRateConfig {
    ...
    pub fn check_request(&self, endpoint: &str, peer_ip: Option<&str>, tokens: u32) -> bool {
        let config = match self.endpoint_configs.get(endpoint) {
            ...
            None => {
                debug!("No rate limit config for endpoint {}, allowing request", endpoint);
                return true;
            }
        };
    ...
    fn check_peer_request(&self, endpoint: &str, peer_ip: &str, config: &EndpointRateConfig, tokens: u32) -> bool {
        ...
        let endpoint_limiters = peer_limiters.entry(endpoint.to_string())
            .or_insert_with(HashMap::new);
        
        let limiter = endpoint_limiters.entry(peer_ip.to_string())
            .or_insert_with(|| RateLimiter::new(&config.algorithm));
        ...
        if !allowed {
            crate::metrics::record_rate_limit_rejection(endpoint);
            warn!("Rate limit exceeded for peer {} on endpoint {}", peer_ip, endpoint);
        }
    ...
    pub fn update_load_multiplier(&self, multiplier: f64) {
        ...
        *load_multiplier = multiplier.clamp(0.1, 10.0);
        ...
        if multiplier < 1.0 {
            info!("System under load, reducing rate limits by factor of {:.2}", multiplier);
        }
    }
    ...
    fn cleanup_unused_limiters(&self) {
        if let Ok(mut peer_limiters) = self.peer_limiters.lock() {
            let initial_count: usize = peer_limiters.values()
                .map(|endpoint_limiters| endpoint_limiters.len())
                .sum();
            ...
            let final_count: usize = peer_limiters.values()
                .map(|endpoint_limiters| endpoint_limiters.len())
                .sum();
            
            if initial_count != final_count {
                info!("Cleaned up {} unused peer rate limiters", initial_count - final_count);
            }
        }
    }
}
