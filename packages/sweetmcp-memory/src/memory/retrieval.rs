    ...
    fn retrieve(
        ...
        tokio::spawn(async move {
            let result: Result<Vec<RetrievalResult>> = (async {
                ...
                // Sort by combined score and take top results - use SmallVec for zero-allocation on small result sets
                let mut sorted_results: SmallVec<[RetrievalResult; 32]> = all_results
                    .into_iter()
...
impl<V: VectorStore + Clone + Send + Sync + 'static> RetrievalManager<V> {
    ...
    pub async fn multi_strategy_retrieve(
        ...
        // Use SmallVec for zero-allocation for small result collections
        let mut all_results: SmallVec<[RetrievalResult; 64]> = SmallVec::new();
