...
impl ContextExtractor {
    ...
    pub async fn extract_embedding(&self, query: &str) -> CognitiveResult<Vec<f32>> {
        // Zero-allocation embedding extraction using ArrayVec for better performance
        use arrayvec::ArrayVec;
        let mut embedding: ArrayVec<f32, 128> = ArrayVec::new();
        
        // Initialize with zeros (zero-allocation)
        for _ in 0..128 {
            embedding.push(0.0);
        }
        
        // Simple embedding extraction (would use proper embedding model in production)
