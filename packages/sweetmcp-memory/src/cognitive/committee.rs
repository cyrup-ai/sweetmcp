...
use crate::cognitive::types::{CognitiveError, OptimizationSpec, Model, ModelType, ImpactFactors};
// LLM types will be used for actual LLM integration when implemented
...
impl EvaluationRubric {
    pub fn from_spec(spec: &OptimizationSpec, user_objective: &str) -> Self {
        ...
        scoring_guidelines.insert(
            "latency".to_string(),
            "Score 0.0-2.0: How much does this change improve speed? (1.0 = no change, <1.0 = faster, >1.0 = slower). Max acceptable: 1.20".to_string()
        );
        scoring_guidelines.insert(
            "memory".to_string(),
            "Score 0.0-2.0: How much does this change affect memory usage? (1.0 = no change, <1.0 = less memory, >1.0 = more memory). Max acceptable: 1.30".to_string()
        );
        scoring_guidelines.insert(
            "relevance".to_string(),
            "Score 0.0-2.0: How much does this improve achieving the objective? (1.0 = no change, >1.0 = better, <1.0 = worse). Min required: 1.40".to_string()
        );
        ...
        Self {
            ...
            success_criteria: vec![
                ...
                "Maintain latency within 20% increase".to_string(),
                "Maintain memory within 30% increase".to_string(),
                "Improve relevance by at least 40%".to_string(),
            ],
...
impl EvaluationCommittee {
    ...
    pub async fn evaluate_action(
        ...
        if initial_consensus.user_satisfaction_impact >= self.consensus_threshold as f32 {
        ...
        if review_consensus.user_satisfaction_impact >= self.consensus_threshold as f32 {
        ...
        if let Some(ref feedback) = steering_feedback {
            ...
            rounds.push(EvaluationRound {
                ...
                consensus: None, // Remove consensus from round tracking for now
                steering_feedback: steering_feedback.clone(),
            });

            if refine_consensus.user_satisfaction_impact >= self.consensus_threshold as f32 {
    ...
    fn generate_steering_feedback(&self, rounds: &[EvaluationRound]) -> Option<String> {
        ...
        // Check if evaluations show consensus on progress
        let progress_votes = latest.evaluations.iter().filter(|e| e.makes_progress).count();
        let makes_progress = progress_votes > latest.evaluations.len() / 2;
        
        // Calculate average confidence from evaluations
        let avg_confidence = latest.evaluations.iter()
            .map(|e| e.objective_alignment)
            .sum::<f64>() / latest.evaluations.len() as f64;

        // If we're making progress with high confidence, no steering needed
        if makes_progress && avg_confidence > 0.7 {
            return None;
        }
        ...
        if !makes_progress {
            ...
            let improvements: Vec<String> = latest.evaluations.iter()
                .flat_map(|e| e.suggested_improvements.iter())
                .take(3)
                .cloned()
                .collect();
            ...
            for eval in &latest.evaluations {
                if eval.makes_progress != makes_progress {
                    feedback.push(format!("- {}: {}", eval.agent_id, eval.reasoning.lines().next().unwrap_or("No reason given")));
                }
            }
        ...
        if avg_confidence < 0.5 {
            feedback.push(format!(
                "\nLow scores indicate issues with alignment ({:.2}), quality, or safety",
                avg_confidence
            ));
        }
    ...
    async fn finalize_evaluation(
        ...
        self.event_tx
            .send(CommitteeEvent::ConsensusReached {
                action: action.to_string(),
                decision: ConsensusDecision {
                    makes_progress: factors.user_satisfaction_impact > 0.5,
                    confidence: factors.user_satisfaction_impact as f64,
                    overall_score: factors.overall_score as f64,
        ...
        info!(
            ...
            action, rounds_taken, factors.user_satisfaction_impact
        );

        Ok(())
    }

    fn calculate_consensus(&self, evaluations: &[AgentEvaluation]) -> ImpactFactors {
        ...
        // Convert consensus to impact factors
        ImpactFactors {
            ...
            // Additional fields required by the struct
            latency_factor: 1.0,
            memory_factor: 1.0,
