...
impl AttentionMechanism {
    ...
    pub async fn calculate_attention_weights(
        ...
        // Use SmallVec for attention scores (typically 8-16 heads) - zero allocation for common cases
        use smallvec::SmallVec;
        let mut all_attention_scores: SmallVec<[Vec<f32>; 16]> = SmallVec::with_capacity(self.num_heads);
        let mut all_weighted_values: SmallVec<[f32; 512]> = SmallVec::with_capacity(self.num_heads * self.head_dim);
        ...
        Ok(AttentionOutput {
            weighted_values: all_weighted_values.into_vec(), // Convert SmallVec to Vec at boundary
            attention_scores: all_attention_scores.into_vec(), // Convert SmallVec to Vec at boundary
            context_vector,
        })
    }
