...
impl<E: Entity + Clone + 'static> EntityRepository for SurrealEntityRepository<E> {
    ...
    fn find_by_type(
        ...
        Box::pin(async move {
            ...
            // Collect results using SmallVec for zero-allocation when results are small
            use smallvec::SmallVec;
            let mut entities: SmallVec<[Box<dyn Entity>; 32]> = SmallVec::with_capacity(limit);
            ...
            // Convert SmallVec to Vec only at the boundary for compatibility
