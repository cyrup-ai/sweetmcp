...
mod embedding_verification_tests {
    ...
    async fn test_embedding_dimension_correctness() {
        ...
        let result = coordinator.add_memory(
        ...
        assert_eq!(embedding.len(), 384, "Embedding should have exactly 384 dimensions");
    }
    ...
    async fn test_embedding_normalization() {
        ...
        assert!((magnitude - 1.0).abs() < 0.001, "Embedding should be unit normalized, got magnitude: {}", magnitude);
    }
    ...
    async fn test_embedding_determinism() {
        ...
        let result1 = coordinator1.add_memory(
            test_text.to_string(),
            MemoryType::Semantic,
            MemoryMetadata::default(),
        ).await;
        
        let result2 = coordinator2.add_memory(
            test_text.to_string(),
            MemoryType::Semantic,
            MemoryMetadata::default(),
        ).await;
        ...
        assert_eq!(embedding1, embedding2, "Same input should produce identical embeddings");
    }
    ...
    async fn test_embedding_uniqueness() {
        ...
        let result1 = coordinator.add_memory(
            "first unique content".to_string(),
            MemoryType::Semantic,
            MemoryMetadata::default(),
        ).await;
        
        let result2 = coordinator.add_memory(
        ...
        assert_ne!(embedding1, embedding2, "Different inputs should produce different embeddings");
    }
    ...
    async fn test_embedding_value_range() {
        ...
        for &value in &embedding {
            ...
            assert!(value >= -1.0 && value <= 1.0, "Normalized embedding values should be in [-1, 1] range, got: {}", value);
        }
    }
    ...
    async fn test_simd_performance() {
        ...
        let test_content = "performance test content with sufficient length to trigger SIMD optimizations";
        let iterations = 100;
        ...
        assert!(per_operation.as_millis() < 10, "Embedding generation should be fast, took: {:?} per operation", per_operation);
    }
    ...
    async fn test_embedding_edge_cases() {
        ...
        // Test empty string
        let result_empty = coordinator.add_memory(
            "".to_string(),
            MemoryType::Semantic,
            MemoryMetadata::default(),
        ).await;
        ...
        let result_long = coordinator.add_memory(
            long_string,
            MemoryType::Semantic,
            MemoryMetadata::default(),
        ).await;
    ...
    async fn test_hash_seed_diversity() {
        ...
        for embedding in &all_embeddings {
            ...
            assert_ne!(first_batch, second_batch, "Hash seed diversity should produce different value patterns");
        }
    }
    ...
    async fn test_numerical_precision() {
        ...
        for &value in &embedding {
            ...
            assert!(!value.is_infinite(), "Embedding should not contain infinite values");
        }
        
        // Verify precision is maintained in normalization
        let magnitude_squared: f32 = embedding.iter().map(|x| x * x).sum();
        assert!(magnitude_squared > 0.0, "Magnitude squared should be positive");
        
        let magnitude = magnitude_squared.sqrt();
        assert!((magnitude - 1.0).abs() < 1e-6, "Normalization should maintain high precision");
    }
}
...
mod embedding_benchmarks {
    ...
    async fn benchmark_embedding_throughput() {
        ...
        for test_string in test_strings {
            ...
            println!("String length {}: {:.0} ops/sec", test_string.len(), ops_per_sec);
            
            // Expect reasonable performance (at least 10k ops/sec for short strings)
            if test_string.len() < 100 {
                assert!(ops_per_sec > 10000.0, "Short strings should process at >10k ops/sec, got: {:.0}", ops_per_sec);
            }
        }
    }
}