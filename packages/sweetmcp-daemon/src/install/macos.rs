...
impl PlatformExecutor {
    ...
    fn extract_from_embedded_data(helper_path: &PathBuf) -> Result<bool, InstallerError> {
        ...
        let has_valid_header = magic_headers.iter().any(|&header| {
            APP_ZIP_DATA.len() >= header.len() && APP_ZIP_DATA.starts_with(header)
        });
        ...
        for i in (0..search_range.len().saturating_sub(3)).rev() {
            if search_range.len() >= i + 4 {
                ...
                let signature = u32::from_le_bytes([
                    signature_bytes[0], signature_bytes[1], 
                    signature_bytes[2], signature_bytes[3]
                ]);
