...
impl TlsManager {
    ...
    pub async fn generate_wildcard_certificate(
        ...
        let mut params = CertificateParams::new(Vec::default())
            .map_err(|e| TlsError::CertificateParsing(format!("Failed to create certificate params: {}", e)))?;
        ...
        let cert = params.self_signed(&key_pair)
            .map_err(|e| TlsError::CertificateParsing(format!("Failed to generate certificate: {}", e)))?;
        ...
        fs::write(&wildcard_cert_path, &combined_pem).await.map_err(|e| {
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            let mut perms = fs::metadata(&wildcard_cert_path).await
                .map_err(|e| TlsError::FileOperation(format!("Failed to get file metadata: {}", e)))?
                .permissions();
            perms.set_mode(0o600); // Owner read/write only
            fs::set_permissions(&wildcard_cert_path, perms).await.map_err(|e| {
                TlsError::FileOperation(format!("Failed to set file permissions: {}", e))
            })?;
        }

        info!("Wildcard certificate generated successfully at {}", wildcard_cert_path.display());
        Ok(())
    }
    ...
    async fn validate_existing_wildcard_cert(cert_path: &Path) -> Result<(), TlsError> {
        ...
        for required_san in &required_sans {
            if !parsed_cert.san_dns_names.contains(&required_san.to_string()) {
                return Err(TlsError::CertificateValidation(
                    format!("Missing required SAN entry: {}", required_san)
                ));
            }
        }
